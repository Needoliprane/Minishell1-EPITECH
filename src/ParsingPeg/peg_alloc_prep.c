...
int peg_count_recurence(char *str, peg_grammar_t *grammar)
{
    ...
    for (int i = 0; str[i]; i++) {
        if (peg_is_grammar(str[i], grammar) == true &&
                    peg_is_grammar(str[i + 1], grammar) == false) {
            count++;
        }
    }
    return (count / 2);
...
peg_grammar_t *peg_set_grammar(char const *synthax_char,
                                char const *allowed_char)
{
                        ...
                        peg_parser_control_t *peg_control)
{
    ...
    for (int i = 0; i <= peg_control->deep; i++) {
        peg_control->peg[i].part1 = peg_malloc_part(128);
        peg_control->peg[i].part2 = peg_malloc_part(128);
    }
    return (peg_control);
}